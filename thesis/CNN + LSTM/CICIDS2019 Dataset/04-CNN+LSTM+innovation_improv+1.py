...
if __name__ == __main__
    ...
    train_path = .datasetsequence_train.data
    val_path = .datasetsequence_val.data
    test_path = .datasetsequence_test.data
    ...
    model = CNN_LSTMNet(input_size, hidden_size=128, output_size=num_classes, num_layers=2, bidirectional=True).to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=0.001)

    run_dir = fresults_cnn_lstm{datetime.now().strftime('%Y%m%d_%H%M%S')}
    ...
    for epoch in range(epochs)
        ...
        print(fEpoch {epoch+1}{epochs} - Train Loss {train_loss.4f} - Val Loss {val_loss.4f})
